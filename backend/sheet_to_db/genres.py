#    genre.guide - From Google Sheets to Redis: genres Python file
...
def build_up_subgenre_information(
    ...
    for subgenre in subgenres:
        ...
        full_data[subgenre]["color"] = dumps(
            genre_to_color[subgenre]) if is_genre else dumps(None)
    ...
    for subgenre, data in full_data.items():
        data["alternative_names"] = dumps(
            tuple(data.get("alternative_names", ())))
...
def create_subgenres_data_set(
        subgenre_data: Dict[str, Dict[str, Any]],
        aliases: Dict[str, str]) -> Dict[str, List[Tuple]]:
    ...
    for alias, points_to in aliases.items():
        actions["subgenre_aliases_to"].append(
            (f"subgenre:alias:{alias}", points_to))

    return actions


async def seed_redis_with_subgenre_data(
        redis: Redis, subgenre_data_set: Dict[str,
    ...
    for index, (subgenre_key_name,
                dictionary) in enumerate(subgenre_data_set["subgenre_by_name"],
                                         start=index + 1):
    ...
    for index, (alias_key_name, points_to) in enumerate(
            subgenre_data_set["subgenre_aliases_to"], start=index + 1):
        if (index % actions_per_transaction) == (actions_per_transaction - 1):
            awaitables.append(transaction.execute())
            # Create a new transaction
            transaction: MultiExec = redis.multi_exec()
