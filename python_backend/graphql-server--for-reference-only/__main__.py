...
class Color(ObjectType):
    ...
    from_genre = Field(
        String, description="The name of the genre that this color comes from")
    foreground = Field(
        String,
        representation=ColorRepresentation(
            name="representation",
            description="The format for the color, such as HEX"),
        description=
        "The text color that appears on top of this color on the Genre Sheet")
    background = Field(
        String,
        representation=ColorRepresentation(
            name="representation",
            description="The format for the color, such as HEX"),
        description="The background color for the genre on the Genre Sheet")

    @staticmethod
    @lru_cache(maxsize=64)
    def _create_tailwind_class_name_for_genre(genre_name: str) -> str:
        words: typing_List[str] = (cast(str, genre_name)).lower().split()
        words_alpha_only: typing_List[str] = [
    ...
    async def _get_hex_colors(genre_name: str) -> Optional[typing_List[str]]:
        ...
        return loads((await
                      do_redis("hget", Subgenre._get_redis_key(genre_name),
                               "color")).decode("utf8"))

    async def resolve_foreground(
        self,
        info,
        representation: ColorRepresentation = ColorRepresentation.HEX):
        if representation == ColorRepresentation.HEX:
            ...
            foreground = (await
                          Color._get_hex_colors(cast(str, self.from_genre)
                                                ))[1].lower()
            ...
            raise GraphQLError(
                f"internal error: the foreground color represented by hex {foreground} is neither white nor black"
            )

        raise GraphQLError(
            f"unknown representation {representation!r} for foreground color")
    ...
    async def resolve_background(
        ...
        if representation == ColorRepresentation.HEX:
            ...
            return Color._create_tailwind_class_name_for_genre(
                cast(str, self.from_genre))

        raise GraphQLError(
            f"unknown representation {representation!r} for background color")
...
class Subgenre(ObjectType):
    ...
    name = String(
        required=True,
        description='The primary name of the subgenre, e.x. "Brostep"')
    alternative_names = List(
        String,
        description=
        'Alternative names for the subgenre, e.x. {"DnB"} for Drum & Bass')
    names = List(
        String,
        description=
        "The primary name of the subgenre, followed by its alternative names")
    is_genre = Boolean(
        required=True,
        description=
        'Whether the subgenre is actually a genre (has its own category and color), e.x. true for Rock, false for Future House'
    )
    genre = Field(
        lambda: Subgenre,
        required=True,
        description=
        "The category that this subgenre belongs to, and that it inherits its color from, e.x. Vaporwave for Vaportrap, Future Bass for Future Bass"
    )
    color = Field(
        Color,
        description=
        "Color information about this subgenre, including the text color it uses on the Genre Sheet and the background color"
    )
    origins = List(
        lambda: Subgenre,
        required=True,
        description=
        'The list of subgenres that this subgenre comes directly from, e.x. {Detroit Techno,} for Big Room Techno, {UK Hip Hop, 2-Step Garage} for Grime'
    )
    subgenres = List(
        lambda: Subgenre,
        required=True,
        description=
        'The list of subgenres who originate directly from this subgenre, e.x. {Deathstep, Drumstep} for Dubstep, {} for Footwork, {Electro Swing, Jazzstep} for Nu-Jazz'
    )
    ...
    async def resolve_alternative_names(self, info):
        return loads(await do_redis("hget",
                                    self._redis_key,
                                    "alternative_names",
                                    encoding="utf8"))
    ...
    async def resolve_is_genre(self, info):
        result: Optional[str] = await do_redis("hget", self._redis_key,
                                               "is_genre")
        return loads(result)

    async def resolve_genre(self, info):
        return Subgenre(name=(
            await do_redis("hget", self._redis_key, "genre")).decode("utf8"))
    ...
    async def resolve_origins(self, info):
        return [
            Subgenre(name=name) for name in loads(await do_redis(
                "hget", self._redis_key, "origins"))
        ]

    async def resolve_subgenres(self, info):
        return [
            Subgenre(name=name) for name in loads(await do_redis(
                "hget", self._redis_key, "subgenres"))
        ]
...
class ReleaseDate(ObjectType):
    datetime = Field(
        ...
        name="datetime",
        description=
        "(internal) datetime object used to determine the other fields")

    iso8601 = Field(
        Date,
        name="iso8601",
        description='The date as an ISO 8601 formatted string, like 2018-04-20'
    )
    year = Field(Int, name="year", description="The year the track released")
    month_name = Field(
        String,
        name="month_name",
        description='The month the track released, as text ("December")')
    month_int = Field(
        Int,
        name="month_int",
        description='The month the track released, as an integer (12)')
    day = Field(Int,
                name="day",
                description="The day of the month the track released")
...
class Track(ObjectType):
    id = Field(ID,
               name="id",
               description="The unique ID describing this track")
    ...
    record_label = Field(
        ...
        name="record_label",
        description=
        "The record label(s) who released and/or own the rights to the track")
    date = Field(ReleaseDate, description="The release date of the track")

    subgenres_json = String(
        name="subgenres_json",
        description=
        "The parsed list of subgenres that make up this song as a JSON string (really low level and provisional)"
    )
    subgenres_flat_json = String(
        name="subgenres_flat_json",
        and_colors=Argument(
            ColorRepresentation,
            required=False,
            description=
            "Include a list of colors in sync with the list of subgenres with this specified representation (see ColorRepresentation for valid options)"
        ),
        description=
        "The subgenres that make up this song, as a flat list (contained in a JSON string). Note that this introduces ambiguity with subgenre grouping"
    )

    image = String(
        name="image",
        description=
        "The link to the cover artwork for the track, or a placeholder")
    ...
    async def resolve_name(self, info):
        return (await do_redis("hget", self._redis_key,
                               "track")).decode("utf8")
    ...
    async def resolve_date(self, info):
        underlying_datetime: datetime = datetime.strptime(
            (await do_redis("hget", self._redis_key,
                            "release")).decode("utf8"), "%Y-%m-%d")
        return ReleaseDate(datetime=underlying_datetime)

    async def resolve_subgenres_json(self, info):
        return (await do_redis("hget", self._redis_key,
                               "subgenre")).decode("utf8")

    async def resolve_subgenres_flat_json(self,
                                          info,
                                          and_colors: Optional[
                                              ColorRepresentation] = None):
        flat_list: typing_List[str] = flatten_subgenres(
            loads(await self.resolve_subgenres_json(info)))
        ...
        for subgenre in map(Subgenre._clean_subgenre_name, flat_list):
            if subgenre in {"|", ">", "~"}:
                ...
                colors.append(
                    (await
                     color_info.resolve_background(info,
                                                   representation=and_colors),
                     await
                     color_info.resolve_foreground(info,
                                                   representation=and_colors)))
            else:
                # The subgenre does not exist
                colors.append(("black", "white") if and_colors ==
                              ColorRepresentation.TAILWIND else ("#000000",
                                                                 "#ffffff"))
...
def all_dates_between(start: date,
                      end: date,
                      reverse: bool = False) -> Generator[date, None, None]:
...
class Query(ObjectType):
    all_subgenres = List(
        Subgenre, description="Retrieve all subgenres from the database")
    subgenre = Field(
        Subgenre,
        name=String(
            description='The name of the subgenre to retrieve, e.x. "Vaporwave"'
        ),
        description="Retrieve a particular subgenre from the database")

    all_genres = List(
        Subgenre,
        description=
        "Retrieve all genres (subgenres with their own color and category) from the database"
    )

    tracks = List(
        Track,
        before=Date(
            description="The newest date of songs to retrieve (inclusive)",
            required=False),
        after=Date(
            description="The oldest date of songs to retrieve (inclusive)",
            required=False),
        newest_first=Boolean(
            description=
            "Whether to sort the returned tracks from newest to oldest, or oldest to newest",
            required=False),
        limit=Int(description="The maximum number of songs to return",
                  required=False),
        before_id=ID(
            description=
            "The newest song to retrieve (exclusive) by its ID. Do not set any other parameter than `limit` when using this",
            required=False),
        description="Retrieve a range of tracks from the Genre Sheet")

    track = Field(Track,
                  id=ID(description="The ID of the track to retrieve"),
                  description="Retrieve a particular track from the database")

    async def resolve_all_subgenres(self, info):
        return [
            Subgenre(name=subgenre.decode("utf8"))
    ...
    async def resolve_subgenre(self, info, name):
        if not (await do_redis("sismember", "subgenres", name)):
            ...
            aliases_to: Optional[bytes] = await do_redis(
                "get", Subgenre._get_redis_key_for_alias(name))
    ...
    async def resolve_all_genres(self, info):
        return [
            Subgenre(name=genre.decode("utf8"))
            for genre in await do_redis("smembers", "genres")
        ]

    async def resolve_tracks(self,
                             info,
                             after=None,
                             before=None,
        ...
        if after is not None:
            for date_ in all_dates_between(after, before,
                ...
                for track_id in await do_redis(
                        "lrange", f"date:{date_.strftime('%Y-%m-%d')}", 0, -1):
            ...
            if not newest_first:
                raise GraphQLError(
                    "`newest_first` must be true when `after` is not specified"
                )
            ...
            if before_id is not None:
                ...
                if not await do_redis("sismember", "tracks", before_id):
                    raise GraphQLError(
                        f"There is no track with id {before_id!r}!")

                track_release_date: str = (await do_redis(
                    "hget", Track._get_redis_key(before_id),
                    "release")).decode("utf8")
                position_in_date_list: int

                for index, other_track_id in enumerate(await do_redis(
                        "lrange", f"date:{track_release_date}", 0, -1)):
                    if other_track_id.decode("utf8") == before_id:
                        position_in_date_list = index
                        break

                # Found the track's position. Yield the songs after it.
                for track_id in await do_redis("lrange",
                                               f"date:{track_release_date}",
                                               position_in_date_list + 1, -1):
                    tracks.append(Track(id=track_id.decode("utf8")))
                ...
                before = datetime.strptime(track_release_date,
...
app.add_route(
    '/graphql',
    GraphQLApp(schema=Schema(query=cast(GraphQLObjectType, Query),
...
if __name__ == '__main__':
    ...
    redis: Redis = loop.run_until_complete(
        create_redis_pool(redis_host, password=redis_password, ssl=ssl))
    with closing(redis):
