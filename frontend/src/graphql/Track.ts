...
import { Arg, Args, ArgsType, ObjectType, Field, FieldResolver, ID, Int, Query, Resolver, ResolverInterface, Root } from "type-graphql";
...
const getTracksFromDateKeys = async function* (dateKeys: Generator<string>): AsyncIterator<string> {
    for (let key of dateKeys) {
        for (let trackID of await client.lrange(key, 0, -1)) {
            yield trackID;
        }
    }
};
...
export class TrackResolver implements ResolverInterface<Track> {
    ...
    async tracks(@Args() { beforeDate, afterDate, beforeID, afterID, newestFirst = true, limit = 50}: TracksArguments) {
